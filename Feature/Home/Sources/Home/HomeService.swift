...
struct HomeService {
  ...
  func fetch() async throws -> [any HomeCellModel] {
    ...
    let getLottoRecommendationTarget = HomeTarget.GetLottoRecommendation(userID: userDataManager.userID)
    async let lottoRecommendationRequest = networkProvider.request(target: getLottoRecommendationTarget)
    ...
    let (lottoRecommendation, dailyFortunes) = try await (lottoRecommendationRequest, dailyFortunesRequest)
    let recommendationCollectionViewCellModel = if let recommendationContent = lottoRecommendation.content {
