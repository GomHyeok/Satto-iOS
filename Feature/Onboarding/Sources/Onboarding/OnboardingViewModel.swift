...
public class OnboardingViewModel : OnboardingViewModelProtocol {
    ...
    public var isNextButtonEnabled: AnyPublisher<Bool, Never> {
        Publishers.CombineLatest3($_isNameValid, $_isBirthDateValid, $_isBornTimeValied).map {(nameValid, birthBalid, bornTimeValid) in
    ...
    func checkBirthFormat(birth : String) -> Bool {
        ...
        if components.count == 3 {
            ...
            if let date = calendar.date(from: dateComponents) {
                ...
                if actualComponents.year != year || actualComponents.month != month || actualComponents.day != day { return false}
            }
        }
