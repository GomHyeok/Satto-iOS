...
public final class SquareTintedChip : UIView {
...
#Preview {
    ...
    SquareTintedChip.Style.allCases.forEach {
        let titleOnly = SquareTintedChip()
        titleOnly.update(text: "Square Tinted Chip")
        titleOnly.update(style: $0)
        stackView.addArrangedSubview(titleOnly)
        let chip = SquareTintedChip()
        chip.update(text: "Square Tinted Chip", icon: STImages.iconArrow.image)
        chip.update(style: $0)
