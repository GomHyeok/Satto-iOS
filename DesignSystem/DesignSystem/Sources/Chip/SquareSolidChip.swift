...
public final class SquareSolidChip: UIView {
...
#Preview {
  ...
  SquareSolidChip.Style.allCases.forEach {
    let titleOnly = SquareSolidChip()
    titleOnly.update(text: "Square Solid Chip")
    titleOnly.update(style: $0)
    stackView.addArrangedSubview(titleOnly)
    let chip = SquareSolidChip()
    chip.update(text: "Square Solid Chip", icon: STImages.iconArrow.image)
    chip.update(style: $0)
