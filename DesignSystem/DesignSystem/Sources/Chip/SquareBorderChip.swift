...
public final class SquareBorderChip : UIView {
    public enum Style : CaseIterable {
        case primary, black, gray, red, orange, yellow, green, blue
        
        fileprivate var backgroudColor : UIColor { return .clear }
...
#Preview {
    ...
    SquareBorderChip.Style.allCases.forEach {
        let titleOnly = SquareBorderChip()
        titleOnly.update(text: "Square Border Chip")
        titleOnly.update(style: $0)
        stackView.addArrangedSubview(titleOnly)
        let chip = SquareBorderChip()
        chip.update(text: "Square Border Chip", icon: STImages.iconArrow.image)
        chip.update(style: $0)
