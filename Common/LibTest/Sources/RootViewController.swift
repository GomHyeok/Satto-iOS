...
public final class RootViewController: UIViewController {
    ...
    @objc private func moveButtonTapped() {
        TestAppRouter.shared.navigate(to: selectedRoute ?? .lib, how: selectedPresentationStyle ?? .push, with: [:])
    }
    ...
    private func showRouteSelectionAlert() {
        ...
        alert.addAction(UIAlertAction(title: "lib", style: .default) { [weak self] _ in
            self?.selectedRoute = .lib
            self?.toButton.setTitle("Lib 화면으로", for: .normal)
        })
        alert.addAction(UIAlertAction(title: "subRoute", style: .default) { [weak self] _ in
    ...
    private func showPresentationSelectionAlert() {
        ...
        // 네비게이션 방식에 대한 액션 추가
        alert.addAction(UIAlertAction(title: "Push", style: .default) { [weak self] _ in
            self?.selectedPresentationStyle = .push
            self?.howButton.setTitle("Push 방식으로", for: .normal)
        })
        alert.addAction(UIAlertAction(title: "Present", style: .default) { [weak self] _ in
        ...
        alert.addAction(UIAlertAction(title: "currentContext", style: .default) { [weak self] _ in
            self?.selectedPresentationStyle = .currentContext
            self?.howButton.setTitle("currentContext 방식으로", for: .normal)
        })
        alert.addAction(UIAlertAction(title: "fullscreen", style: .default) { [weak self] _ in
            self?.selectedPresentationStyle = .fullscreen
            self?.howButton.setTitle("fullscreen 방식으로", for: .normal)
        })
        alert.addAction(UIAlertAction(title: "overCurrentContext", style: .default) { [weak self] _ in
            self?.selectedPresentationStyle = .overCurrentContext
            self?.howButton.setTitle("overCurrentContext 방식으로", for: .normal)
        })
        alert.addAction(UIAlertAction(title: "overFullScreen", style: .default) { [weak self] _ in
